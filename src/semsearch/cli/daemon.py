...
from semsearch.cli.ingest.crawl_jobs import (
    ProcessNextCrawlJob,
    create_crawl_job_processor,
)
    ...
    async with lock(pool, DAEMON_LOCK_ID):
        ...
        async with asyncio.TaskGroup() as tasks:
            ...
            for _ in range(settings.ingest_concurrency):
                tasks.create_task(_crawl_loop(process_next))
...
async def _crawl_loop(process_next: ProcessNextCrawlJob) -> None:
    while True:
        try:
